...
var (
	flagInputFile  = flag.String("i", "-", "Input ebcdic file (use '-' for stdin)")
	flagCodepage   = flag.String("c", "037", "EBCDIC-Codepage to use (supported: 037 / 273 / 500 / 1140 / 1141 / 1148)")
	flagOutputFile = flag.String("o", "-", "Output utf8 file (use '-' for stdout)")
...
func main() {
	...
	for scanner.Scan() {
		...
		outputData, err := ebcdic.Decode(inputData, codePage)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error encoding data: %s\n", err)
		}

		_, err = writer.Write([]byte(outputData))
		if err != nil {
